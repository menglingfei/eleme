<template>
  <div class="goods">
    <div class="menu-wrap" ref="menuWrapper">
      <div class="menu-list">
        <div class="menu-item" :class="currentIndex === $index?'current':''" :key="item.id" v-for="(item, $index) in goods">
          <span class="menu-item-text border-1px">
            <span class="menu-item-icon" v-show="item.type>0" :class="classMap[item.type]"></span>
            {{item.name}}
          </span>
        </div>
      </div>
